...
**v2 (2026-09-10): the same bake on [MikeCaldera's fresh GPTQ quant](https://huggingface.co/mikeinnyc/Qwen3.8-27B-GPTQ-Int4-sym-G128-MTP-BF16) of the same base model** — same INT4 g128 layout, so the pipeline ran unchanged, and layer errors match v1 within 0.1 points. Published as [`Launch80/Qwen3.8-27B-GPTQ-Int4-baked-v2`](https://huggingface.co/Launch80/Qwen3.8-27B-GPTQ-Int4-baked-v2). BetterBench weighted decode **73.5 tok/s** vs 71.8 for v1, same config and harness ([run page](https://launch80.com/a/0a807aa7-36a2-4ba8-83c0-12f06ef4031b), [`results/bake-v2-betterbench-k6.json`](results/bake-v2-betterbench-k6.json)). The two read identical bytes per step, so the +2.4% is draft acceptance from the body. Worth knowing: a repetitive counting prompt had shown v2 ~30% ahead; on BetterBench's mixed corpus that shrinks to 2%. Acceptance gains are workload-shaped, which is why only the harness number is quoted. **v2 has not been through the quality gates** (perplexity, paired GSM8K, greedy invariant); v1's results do not transfer to a different body, and v2 stays a candidate until they are in.

**Embed-INT8 derivative (2026-09-13):** baked-v2 still left `embed_tokens` FP16 (~2.37 GiB). An INT8-only derivative reclaims ~1.15 GiB of resident weights and returns that to FP8 KV (~65k → ~131k-class window on a 24 GiB B60) with short decode within ~1% of the FP16-embed baseline under graphs+MTP6. INT4 embed was measured and **rejected** (−21% decode). This path still needs a small boot patch (stock vLLM has no INT8 embed method). See [`docs/embed-int8.md`](docs/embed-int8.md), [`bake/bake_embed_quant.py`](bake/bake_embed_quant.py), [`patches/patch_embed_int8.py`](patches/patch_embed_int8.py).
