...
template <int SG, int SGS> class W4Kernel;

template <int SG, int SGS>
static void launch_w4(sycl::queue &q, const half_t *x, const int32_t *qw,
    ...
    q.submit([&](sycl::handler &h) {
        h.parallel_for<W4Kernel<SG, SGS>>(
            ...
            [=](sycl::nd_item<1> it) [[sycl::reqd_sub_group_size(SG)]] {
                ...
                const half_t  *scn = sc + n * (size_t)NG;
                const half2_t zp{(half_t)1032.0f, (half_t)1032.0f};   // 1024 + 8
                float acc = 0.f;

                for (int u = lane; u < NV; u += SG) {
                    const v4_t wv = *reinterpret_cast<const v4_t *>(col + (size_t)u * 4);
                    half2_t s2{(half_t)0.f, (half_t)0.f};
#pragma unroll
                    for (int e = 0; e < 4; ++e) {
                        const uint32_t w = (uint32_t)wv[e];
                        const h8_t xv =
                            *reinterpret_cast<const h8_t *>(x + (size_t)u * 32 + e * 8);
#pragma unroll
                        for (int t = 0; t < 4; ++t) {
                            const uint32_t bits =
                                ((w >> (4 * t)) & 0x000F000Fu) | 0x64006400u;
                            const half2_t q = sycl::bit_cast<half2_t>(bits) - zp;
                            const half2_t xp{xv[2 * t], xv[2 * t + 1]};
                            s2 += q * xp;
                        }
                    }
                    // one scale per 128 weights = per 4 units; u*32 is inside one group
                    acc += ((float)s2[0] + (float)s2[1]) * (float)scn[u / 4];
                }
                acc = sycl::reduce_over_group(sg, acc, sycl::plus<float>());
                if (lane == 0) out[n] = (half_t)acc;
            });
    });
}
...
at::Tensor gemv_w4(const at::Tensor &x, const at::Tensor &qw, const at::Tensor &sc) {
    ...
    launch_w4<32, 4>(q, xp, qw.data_ptr<int32_t>(), scp, op, K, N);
    return out;
}

TORCH_LIBRARY(p608, m) {
    m.def("gemv_w4(Tensor x, Tensor qw, Tensor sc) -> Tensor");
}
